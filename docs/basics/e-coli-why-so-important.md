...
### 1-3. 「20分より速く」はなぜ難しいのか

- ポイント: 20分は最適条件下での目安で、ここから大きく短縮するのは生物物理的な下限に近いため難しいです。
- 主な理由（エンジニア向け直感）:
  - DNA複製の所要時間: 複製には最低時間が必要で、多重起点で先行して複製を進めても物理限界があります。
  - たんぱく質合成のスループット: リボソームの生産・翻訳速度、tRNA供給などのパイプラインに上限があります。
  - リソース配分のトレードオフ: 代謝エネルギーと前駆体を「増えるための機械（リボソーム等）」と「その他の機能」に割り当てる必要があり、過度の高速化は他機能を圧迫して適応度を落とします。
  - 細胞表層の製造速度: 膜・細胞壁の合成や表面積拡大にも速度制約があります。
- 研究の現状: 何が「絶対的な下限」を決めているかは完全には解明されておらず、実験条件や株によって最短時間はわずかに変動しますが、実用的には約20分前後が下限に近いと考えられています。
...
1. **糖鎖修飾ができない**
   ...
   - 大腸菌にはその機能がない
   - 実務での回避策:
     - 哺乳類細胞で発現（例: CHO, HEK293）→ ヒト型に近い糖鎖
     - 糖鎖を設計した酵母・昆虫細胞（グリコエンジニアリング）
     - 試験管内での酵素的/化学的グリコシル化（in vitro, cell-free）
     - 大腸菌への糖鎖経路導入（例: C. jejuniのN結合経路）も研究・実装例あり。ただし糖鎖の種類やパターンは限定的
...
3. **大腸菌にとって毒性のあるタンパク質は条件次第**
   - 正確には「まったく作れない」のではなく、強い毒性があると高発現で増殖阻害や細胞死を招き、生産が難しくなるという意味
